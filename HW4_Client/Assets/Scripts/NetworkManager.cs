...
public class NetworkManager : MonoBehaviour
{
	...
	public bool SendTestRequest( int x)
	{
		if (cManager && cManager.IsConnected())
		{
		...
		public bool SendColorChangeRequest(int pieceIndex)
	{
		if (cManager && cManager.IsConnected())
		{
			RequestColorChange request = new RequestColorChange();
			request.send(pieceIndex);
			cManager.send(request);
			return true;
		}
		return false;
	}
	public bool SendWinRequest()
	{
		if (cManager && cManager.IsConnected())
		{
			Debug.Log("winRequestSend");
			RequestWin request = new RequestWin();
			cManager.send(request);
			return true;
		}
		return false;
